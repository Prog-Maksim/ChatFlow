...
public class MessageRepository: IMessageRepository
{
    ...
    private readonly IMongoCollection<ChatData> _chatCollections;
    ...
    public MessageRepository(IMongoClient client, ILogger<MessageRepository> logger)
    {
        ...
        var chatDatabase = client.GetDatabase("Chats");
        var messageDatabase = client.GetDatabase("Messages");
        
        _chatCollections = chatDatabase.GetCollection<ChatData>("chats");
    ...
    {
        await _chatCollections.InsertOneAsync((ChatData)@event);
    }

    public async Task<ChatData?> GetChatAsync(string chatId)
    {
        var filter = Builders<ChatData>.Filter.Eq(chat => chat.ChatId, chatId);
